...
class Edgenet_Cat {
	...
	public function __construct() {
		...
		add_filter( 'manage_edit-edgenet_cat_columns', [ $this, 'manage_terms_columns' ] );

		add_action( 'manage_edgenet_cat_custom_column', [ $this, 'custom_column' ], 10, 3 );
	}
	...
	public function edit_category_fields( $term ) {
		...
		$data['term_id'] = absint( $_REQUEST['tag_ID'] ); // phpcs:ignore
	...
	/**
	 * Save WooCommerce Product Categories Fields.
	 *
	...
	public function save_category_fields( $term_id, $tt_id, $taxonomy ) {
		...
		// Sanitize input.
		foreach ( $_POST[ self::META_EDGENET_2_PRODUCT ] as $val ) { // phpcs:ignore
	...
	public function render_term_tree_checkboxes( $tree, $selected = [], $level = 0 ) {
		...
		foreach ( $tree as $branch ) {
			// Generate HTML input type=checkbox.
			...
			// If we have child terms, recurse into each of them.
	...
	/**
	 * Create a column in the Term List Table to display linked Product Categories.
	 *
	 * @param array $columns The existing Term List Table columns.
	 *
	 * @return array The updated array of table columns.
	 */
	...
	/**
	 * Populate Product Category column for each Term.
	 *
	 * @param string $content     Column content.
	 * @param string $column_name The column key.
	 * @param int    $term_id     The Term ID.
	 *
	 * @return string Content to display for this term.
	 */
	public function custom_column( $content, $column_name, $term_id ) {

		if ( 'product_cats' === $column_name ) {
			$linked_terms = json_decode( get_term_meta( $term_id, self::META_EDGENET_2_PRODUCT, true ) );
			if ( empty( $linked_terms ) ) {
				$content .= '<span aria-hidden="true">—</span>';
			} else {
				foreach ( $linked_terms as $linked_term ) {
					$term = get_term( $linked_term );

					$content .= $term->name . '<br />';
				}
			}
		}
